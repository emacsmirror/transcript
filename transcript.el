...
;; Yahwe is Yet Another Harmless .Wav Extractor usable for WAV files
;; in transcript mode. I'm sure there are dozens of other programs
;; with the same abilities, but this one is mine. Yahwe is not included
;; in this file.
(defvar tm-yahwe-protocol
  '((program . "yahwe")
    (options . "-s")
    (filter . tm-yahwe-filter)
    (max  . "max?\n")
    (pos  . "pos?\n")
    (play . "play\n")
    (skip . "skip %d\n")
    ...
    (quit . "quit\n")
    (seek . "seek %d\n"))
  "The protocol spoken by yahwe."
  )

(defun tm-yahwe-filter (process line)
  "Interpret yahwe output."
  ;; yahwe output is very spartanic:
  ;; either `00:00:01' to show the current position...
  (if (string-match ":" line)
      (progn
        ...
        (setq tm-current-second (tm-parse-time line))
        (tm-draw-modeline))
    ;; ...or `4651' after a max? request.
    (setq tm-file-length (string-to-int line))
    ))
